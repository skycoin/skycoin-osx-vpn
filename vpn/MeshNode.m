...
- (void)start {
    ...
    stdErrPipe.fileHandleForReading.readabilityHandler = ^(NSFileHandle *readableHandle) {
        ...
        NSLog(@"----- Node output: %@", [[NSString alloc] initWithData:readableHandle.availableData encoding:NSUTF8StringEncoding]);
#endif
    };
...
- (void)sendMessage:(MeshMessage *)message {
    ...
    uint32_t length = htonl((uint32_t)[serialized length]);
